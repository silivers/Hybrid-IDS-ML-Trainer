...
def preprocess_for_xgboost(df, fit_encoders=True, encoders=None, scaler=None):
    ...
    # 2. 处理缺失值（XGBoost可以处理缺失值，但填充0更安全）
    ...
    for col in CATEGORICAL_COLS:
        if col in df_processed.columns:
            if fit_encoders:
                ...
                log_message(f"编码特征: {col} (类别数: {len(encoders[col].classes_)})")
            else:
                # 处理测试集中可能出现的新类别
                ...
                if unknown_mask.any():
                    log_message(f"警告: 测试集'{col}'中发现{unknown_mask.sum()}个未知类别，将编码为-1", "WARNING")
...
def main():
    ...
    if y_train is not None:
        log_message(f"训练集标签分布: 正常={np.sum(y_train==0)} ({np.sum(y_train==0)/len(y_train)*100:.2f}%), "
                   f"攻击={np.sum(y_train==1)} ({np.sum(y_train==1)/len(y_train)*100:.2f}%)")
    
    if y_test is not None:
        log_message(f"测试集标签分布: 正常={np.sum(y_test==0)} ({np.sum(y_test==0)/len(y_test)*100:.2f}%), "
                   f"攻击={np.sum(y_test==1)} ({np.sum(y_test==1)/len(y_test)*100:.2f}%)")
    
    # 数据质量检查
    log_message(f"\n数据质量检查:")
    log_message(f"训练集特征范围: [{X_train.min():.4f}, {X_train.max():.4f}]")
    log_message(f"测试集特征范围: [{X_test.min():.4f}, {X_test.max():.4f}]")
    
    # 检查是否有NaN
    if np.isnan(X_train).any() or np.isnan(X_test).any():
        log_message("警告: 数据中存在NaN值", "WARNING")
    else:
        log_message("✓ 无NaN值")
    ...
    # 可选：保存预处理后的数据（加速后续训练）
    # np.savez(MODELS_DIR / "preprocessed_data.npz", 
