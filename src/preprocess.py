...
def inspect_data(df, name="Dataset"):
    ...
    log_message(f"  列名: {df.columns.tolist()[:10]}...")  # 只显示前10列
...
def safe_label_encode(train_series, test_series, col_name):
    ...
    def encode_test_value(val):
        ...
        if val in known_classes:
            ...
            unknown_count += 1
            log_message(f"  测试集列 '{col_name}' 中发现未知类别 '{val}'，编码为 -1", "WARNING")
            return -1
...
def preprocess_data_safe(train_df, test_df):
    """
    安全的数据预处理
    """
    log_message("\n开始安全数据预处理...")
...
def preprocess_and_cache():
    ...
    # 安全预处理
    X_train, X_test, y_train, y_test, encoders, scaler = preprocess_data_safe(train_df, test_df)
